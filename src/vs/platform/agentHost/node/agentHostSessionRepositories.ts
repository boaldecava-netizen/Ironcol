 ...
 * easy to unit test.
 */
export async function resolveSessionRepositories(workingDirectories: readonly URI[], gitService: IAgentHostGitService): Promise<ISessionRepositories> {
	const resolvedRoots = await Promise.all(workingDirectories.map(async workingDirectory => ({
		workingDirectory,
		repositoryRoot: await gitService.getRepositoryRoot(workingDirectory),
	})));
